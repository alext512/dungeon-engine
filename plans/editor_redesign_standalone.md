...
| `puzzle_dungeon/editor/editor_app.py` | Main editor application (~800-1000 lines) |

### Modified Files
| File | Changes |
|------|---------|
| `puzzle_dungeon/editor/level_editor.py` | Simplify: remove `handle_events()`/`update()`, keep all data methods (tile painting, entity ops, save/load) |
| `puzzle_dungeon/engine/game.py` | Remove all editor code, accept area_path, add level selection |
| `puzzle_dungeon/engine/renderer.py` | Revert to play-mode only, remove editor_ui parameter |
| `puzzle_dungeon/config.py` | Add `EDITOR_WIDTH`, `EDITOR_HEIGHT` constants |

### Deleted Files
| File | Reason |
|------|--------|
| `puzzle_dungeon/editor/editor_ui.py` | Replaced by `editor_app.py` |
| `puzzle_dungeon/editor/browser_window.py` | No longer needed |
