...
2. **Command runner**: resolves JSON commands and drives runtime behavior. `CommandContext` carries runner/project state, while `CommandServices` owns injected runtime dependencies like `world`, `camera`, dialogue/inventory runtimes, persistence, and scene-change hooks. Registry injection resolves explicit command parameters from that service bundle. See `dungeon_engine/commands/runner.py`.
